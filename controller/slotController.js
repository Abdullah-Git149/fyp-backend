...
const updateSlot = async (req, res) => {
  try {
    ...
    if (check.length>0) {
       return res.status(400).json({ status: 0, msg: `Slot from ${check[0].st_time} to ${check[0].end_time} has been booked already` })
    } else {
...
const classList = async (req, res) => {
  try {
    if (!req.body.class_name) {
      ...
      return res.status(400).json({ status: 0, message: "Please select a class" });
    } else {

      const slot = await Slot.find({ class_name: req.body.class_name })
      if (slot.length > 0) {
        return res.status(200).json({ status: 1, msg: "List of Slots", slots: slot })
      } else {
        return res.status(400).json({ status: 0, msg: "No Slot Found of this class" })
...
module.exports = { addSlot, listOfSlots, deleteSlot, updateSlot, classList };
